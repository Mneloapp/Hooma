...
import { UniversalImportForm } from "@/components/admin/MakerWorldImportForm";
...
export default async function ImportInboxPage() {
  ...
  const { data, error } = supabase
    ? await supabase.from("source_imports").select("id,source_url,platform,source_title,source_model_id,status,extracted_metadata,error_message,created_at").order("created_at", { ascending: false }).limit(50)
    : { data: [], error: null };
  const imports = (data ?? []) as ImportRow[];

  return (
    <div className="space-y-6">
      <div><p className="text-xs uppercase tracking-[0.28em] text-hooma-muted">Catalog sources</p><h1 className="mt-3 text-4xl font-medium">უნივერსალური იმპორტი</h1><p className="mt-3 max-w-3xl text-sm leading-6 text-hooma-muted">ჩასვი მხარდაჭერილი 3D მოდელების პლატფორმის ბმული — Hooma წაიკითხავს JSON-LD/Open Graph metadata-ს და შექმნის მონახაზს.</p></div>
      ...
      <section className="overflow-hidden rounded-[1.5rem] bg-white/75 shadow-soft">
        ...
        {!error && !imports.length ? <div className="flex flex-col items-center px-6 py-16 text-center"><div className="grid h-12 w-12 place-items-center rounded-2xl bg-hooma-panel"><ArrowRight size={19} /></div><p className="mt-5 font-medium">ჯერ არცერთი ბმული არ დაგიმატებია</p><p className="mt-2 text-sm text-hooma-muted">პირველი 3D მოდელის პროდუქტის ბმული ზემოთ ჩასვი.</p></div> : null}
      </section>
    </div>
  );
}
