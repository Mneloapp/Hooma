...
export default async function AdminSettingsPage({ searchParams }: { searchParams: SearchParameters }) {
  ...
  const validProvider = provider === "instagram" || provider === "tiktok"
    ? provider
    : null;
  const validResult = validProvider
  ...
  return <div className="space-y-6">
    ...
    {isOwner ? <section className="rounded-3xl border border-hooma-line bg-white p-6 shadow-sm">
      ...
      <p className="mt-2 text-sm leading-6 text-hooma-muted">ანგარიშის დაკავშირება გამოქვეყნებას არ რთავს — ავტომატური გამოქვეყნება ცალკე, ნაგულისხმევად გამორთული კონტროლია.</p>
      <div className="mt-5 grid gap-4 lg:grid-cols-2">
        ...
        <div className="rounded-2xl border border-hooma-line p-5">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <h2 className="text-2xl font-medium">TikTok</h2>
              {tiktokConnection ? <p className="mt-2 text-sm text-hooma-muted">
                @{tiktokConnection.username} · {tiktokConnection.status === "active" ? "დაკავშირებულია" : "ხელახალი ავტორიზაციაა საჭირო"}
              </p> : <p className="mt-2 text-sm text-hooma-muted">მხოლოდ @hooma.ge ანგარიში შეიძლება დაუკავშირდეს.</p>}
              {validProvider === "tiktok" && validResult === "connected" ? <p className="mt-2 text-sm text-emerald-700">TikTok წარმატებით დაუკავშირდა.</p> : null}
              {validProvider === "tiktok" && validResult && validResult !== "connected" ? <p className="mt-2 text-sm text-rose-700">TikTok-ის ავტორიზაცია ვერ დასრულდა ({validResult}). ხელახლა სცადე მხოლოდ @hooma.ge ანგარიშით.</p> : null}
              {!canConnectTikTok ? <p className="mt-2 text-sm text-amber-800">დაკავშირება ჩაირთვება მხოლოდ აპის დამტკიცებისა და ზუსტი scope-ების დაფიქსირების შემდეგ.</p> : null}
            </div>
            {canConnectTikTok ? <a
              href="https://hooma.ge/api/social/oauth/tiktok/start"
              className="inline-flex min-h-11 items-center justify-center rounded-full bg-hooma-ink px-5 py-3 text-sm font-medium text-white"
            >
              {tiktokConnection?.status === "active" ? "TikTok-ის ხელახლა დაკავშირება" : "TikTok-ის დაკავშირება"}
            </a> : <span className="inline-flex min-h-11 items-center justify-center rounded-full bg-hooma-line px-5 py-3 text-sm font-medium text-hooma-muted" aria-disabled="true">
              TikTok ჯერ მზადდება
            </span>}
          </div>
      ...
      {connectionResult.error ? <p className="mt-4 text-sm text-amber-800">Social connection-ის საცავი ჯერ არ არის გააქტიურებული.</p> : null}
    </section> : null}
