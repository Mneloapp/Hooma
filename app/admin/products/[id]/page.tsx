...
import { DraftProductEditor } from "@/components/admin/DraftProductEditor";
...
export default async function AdminProductDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [productResult, costResult, operatorReferenceResult, categoryResult, materialResult, storefrontCardResult] = await Promise.all([
    ...
    supabase.from("material_cost_profiles").select("id,code,name").eq("is_active", true).order("code"),
    supabase.from("storefront_product_cards").select("product_id").eq("product_id", id).maybeSingle(),
  ]);
  ...
  const draftColors = amsRequired
    ? fixedColorPalette
    : Array.isArray(variant?.available_colors)
      ? variant.available_colors.filter((color: unknown): color is string => typeof color === "string")
      : [];
  const categoryOptions = buildCategoryOptions((categoryResult.data ?? []) as CategoryRow[]);
