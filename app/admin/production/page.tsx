...
export default async function ProductionPage({
  ...
  const { data: itemRows, error: itemError } = admin && orderIds.length
    ? await admin.from("order_items").select("id,order_id,product_name,sku,size_label,material,color,quantity").in("order_id", orderIds).order("created_at")
    : { data: [], error: null };
  ...
  const errorsPresent = printerError || orderError || itemError || jobError;
  ...
  return (
    <div className="space-y-7">
      ...
      <section className="rounded-[1.5rem] border border-blue-200 bg-blue-50 p-5 text-blue-950">
        <div className="flex items-start gap-3"><CircleAlert size={20} className="mt-0.5 shrink-0" /><div><h2 className="font-semibold">V1 სამუშაო წესი — თანმიმდევრობა მნიშვნელოვანია</h2><p className="mt-2 text-sm leading-6 text-blue-900/80"><strong>ჯერ Hooma-ში დაჯავშნე პრინტერი.</strong> მხოლოდ წარმატებული ჯავშნის შემდეგ გახსენი MakerWorld ბმული და გაუშვი სწორი ფერი/მასალა Bambu Studio-დან. როცა ფიზიკური ბეჭდვა ნამდვილად დაიწყება, მეორე ნაბიჯზე დაადასტურე დაწყება. ასე ორი ოპერატორი ერთ სამუშაოს ორჯერ ვერ გაუშვებს. მომხმარებელს ეს შიდა მოქმედებები არ უჩანს.</p></div></div>
      </section>
            ...
            return (
              <article key={job.id} className="rounded-[1.5rem] border border-hooma-text/10 bg-white/80 p-5 shadow-sm">
                ...
                <div className="mt-4 flex flex-wrap items-center gap-3 text-sm">
                  {sourceUrl ? <a href={sourceUrl} target="_blank" rel="noreferrer noopener" className="inline-flex items-center gap-2 rounded-full border border-hooma-text/15 bg-white px-4 py-2 font-semibold hover:border-hooma-accent"><ExternalLink size={15} />MakerWorld-ის გახსნა</a> : <span className="inline-flex items-center gap-2 rounded-full bg-amber-100 px-4 py-2 text-xs font-semibold text-amber-900"><CircleAlert size={15} />Custom/Test ფაილი გადაამოწმე</span>}
                  <span className="text-xs text-hooma-muted">3MF profile: {job.print_profile_path ? "დამაგრებულია" : "წყაროს გვერდიდან"}</span>
                </div>
          ...
          {preparingJobs.map((job) => {
            ...
            return (
              <article key={job.id} className="rounded-[1.5rem] border border-violet-200 bg-violet-50 p-5">
                ...
                <div className="mt-4 flex flex-wrap items-center gap-3">{sourceUrl ? <a href={sourceUrl} target="_blank" rel="noreferrer noopener" className="inline-flex items-center gap-2 rounded-full border border-violet-200 bg-white px-4 py-2 text-sm font-semibold"><ExternalLink size={15} />MakerWorld-ის გახსნა</a> : <span className="rounded-full bg-amber-100 px-4 py-2 text-xs font-semibold text-amber-900">Custom/Test ფაილი გადაამოწმე</span>}<span className="text-xs text-violet-900/70">ჯერ გაუშვი ამ დაჯავშნილ პრინტერზე Bambu Studio-დან.</span></div>
