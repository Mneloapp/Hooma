...
const getProductBySlug = cache(getStorefrontProductBySlug);

type ProductPageParams = { slug: string };
type ProductPageSearchParams = { preview?: string };

export async function generateMetadata({ params, searchParams }: {
  params: Promise<ProductPageParams>;
  searchParams: Promise<ProductPageSearchParams>;
}): Promise<Metadata> {
  const [{ slug }, { preview: previewId }] = await Promise.all([params, searchParams]);
  const product = previewId
    ? await getAdminPreviewProductById(previewId)
    : await getProductBySlug(slug);
  if (!product || product.slug !== slug) return privatePageMetadata;

  const metadata = publicPageMetadata({
    title: product.nameKa,
    description: compactDescription(product.shortDescriptionKa || product.shortDescription),
    path: `/product/${encodeURIComponent(product.slug)}`,
    image: product.heroImage,
    index: !previewId,
  });
  return previewId ? { ...metadata, robots: privatePageMetadata.robots } : metadata;
}

export default async function ProductPage({ params, searchParams }: { params: Promise<ProductPageParams>; searchParams: Promise<ProductPageSearchParams> }) {
  ...
  const product = previewProduct ?? await getProductBySlug(slug);
  if (!product) notFound();
  ...
  const categoryUrl = categoryPath(product.categorySlug);
  const productUrl = absoluteUrl(`/product/${encodeURIComponent(product.slug)}`);
  const productImages = Array.from(new Set([product.heroImage, ...product.galleryImages])).map(absoluteUrl);
  const offerVariant = activeDeal
  ...
  const productJsonLd = !previewProduct && offerPrice !== null ? {
    ...
    name: product.nameKa,
    alternateName: product.hoomaName !== product.nameKa ? product.hoomaName : undefined,
    description: compactDescription(product.shortDescriptionKa || product.shortDescription),
    image: productImages,
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: localizedCategory?.nameKa ?? product.category, item: absoluteUrl(categoryUrl) },
      { "@type": "ListItem", position: 4, name: product.nameKa, item: productUrl },
    ],
  };
  ...
  return (
    <main className="mx-auto max-w-[1480px] px-4 py-6 sm:px-6 lg:px-8">
      ...
      {!previewProduct ? <JsonLd data={breadcrumbJsonLd} /> : null}
      ...
      <section className="grid items-start gap-7 lg:grid-cols-[minmax(0,0.95fr)_minmax(0,0.9fr)_330px] xl:gap-10">
        ...
        <div className="min-w-0">
          <Link href={categoryUrl} className="text-xs font-semibold uppercase tracking-[0.16em] text-hooma-accent hover:underline"><LocalizedText ka={localizedCategory?.nameKa ?? product.category} en={localizedCategory?.name ?? product.category} /></Link>
      ...
      <div className="mt-14"><ProductShelf title="მსგავსი პროდუქტები" titleEn="Similar products" products={recommendations.map((item) => applyProductCardDeal(item, dailyDealByProductId.get(item.id)))} href={categoryUrl} /></div>
    </main>
  );
}
