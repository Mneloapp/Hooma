        ...
        <div className="mt-5 grid gap-4">
          {requests.length ? requests.map((request) => (
            <article key={request.id} className="rounded-2xl border border-hooma-text/10 bg-hooma-background p-5">
              ...
              {request.status === "payment_pending" ? <div className="mt-4 rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900"><p className="font-semibold"><LocalizedText ka="ინდივიდუალური შეკვეთის ონლაინ გადახდა ჯერ მიუწვდომელია" en="Online payment for custom orders is not available yet" /></p><p className="mt-1 text-xs leading-5"><LocalizedText ka="საქართველოს ბანკის ონლაინ checkout ამ ეტაპზე მხოლოდ კატალოგის შეკვეთებზე მუშაობს. ამ მოთხოვნაზე თანხა ავტომატურად არ ჩამოგეჭრება; შემდეგ ნაბიჯს ოპერატორი დაგიდასტურებს." en="Bank of Georgia online checkout currently applies only to catalog orders. You will not be charged automatically for this request; an operator will confirm the next step." /></p></div> : null}
