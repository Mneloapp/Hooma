...
const amountOrNull = (minor: number | null) => minor === null ? null : minorToAmount(minor);
...
export async function POST(request: Request) {
  ...
  const callbackOrderId = typeof callbackBody?.order_id === "string" ? callbackBody.order_id.trim() : "";
  ...
  try {
    // The signed callback proves provenance. The fresh receipt is the
    // authoritative snapshot used for amount/status reconciliation.
    ...
    const { data, error } = await admin.rpc("apply_bog_payment_result_v1", {
    ...
    if (error) {
      console.error("BOG_CALLBACK_RECONCILIATION_FAILED", {
    ...
    if (result?.processing_status === "manual_review") {
      console.error("BOG_CALLBACK_REQUIRES_REVIEW", {
        providerOrderId: details.orderId,
    ...
    console.error("BOG_CALLBACK_RECEIPT_FAILED", {
