...
async function productPageAvailable(item: TikTokNineDayCampaignItem) {
...
async function verifyStagedObject(
  ...
  if (new URL(signedUrl).origin !== new URL(socialMediaBaseUrl()).origin) {
...
function exactExistingJob(
  ...
  item: TikTokNineDayCampaignItem,
  accountId: string,
  productId: string,
) {
  return row.post_id === item.postId
    ...
    && stableJson(row.music_receipt) === stableJson(tiktokNineDayMusicReceipt(item))
    && stableJson(row.settings) === stableJson(tiktokNineDaySettings(item))
    && row.idempotency_key === `tiktok-campaign:${item.postId}:${item.videoSha256}`;
}
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const item = tiktokNineDayCampaignItem(body.postId);
    if (!item) return response(404, { ok: false, status: "CAMPAIGN_ITEM_NOT_FOUND" });
    if (sha256(item.caption) !== item.captionSha256) throw new Error("CAPTION_HASH_MISMATCH");
    ...
    const read = await admin.from("social_publish_jobs")
      .select(selectedFields).eq("post_id", item.postId).maybeSingle();
    ...
    if (!existing) {
      const insert = await admin.from("social_publish_jobs").insert({
        ...
        music_receipt: tiktokNineDayMusicReceipt(item),
        music_receipt_sha256: ZERO_SHA256,
        rights_status: "CLEARED",
        visual_claims_status: "CLEARED",
        settings: tiktokNineDaySettings(item),
        settings_sha256: ZERO_SHA256,
        content_fingerprint: ZERO_SHA256,
        idempotency_key: `tiktok-campaign:${item.postId}:${item.videoSha256}`,
        max_attempts: 5,
        created_by: actor.id,
    ...
    console.error(JSON.stringify({
      level: "error",
      message: "tiktok_nine_day_campaign_finalize_failed",
      errorCode,
    }));
