  ...
  const filtered = useMemo(
    () =>
      localRows.filter((row) => {
        const matchesQuery = `${row.product_name} ${row.sku} ${row.color} ${row.fabric}`.toLowerCase().includes(query.toLowerCase());
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-col gap-3 rounded-[1.5rem] bg-white/70 p-4 sm:flex-row">
        <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search product, SKU, color, fabric" className="min-h-11 flex-1 rounded-full border border-hooma-text/10 bg-white px-4 text-sm outline-none focus:border-hooma-accent" />
            ...
            <tbody className="divide-y divide-hooma-text/10">
              {filtered.map((row) => (
                <tr key={row.id}>
                  ...
                  <td className="px-5 py-4 text-hooma-muted">{row.fabric} / {row.color} / {row.orientation}</td>
