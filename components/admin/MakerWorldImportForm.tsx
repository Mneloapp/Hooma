  ...
  return (
    <form action={action} className="rounded-[2rem] bg-white/75 p-6 shadow-soft">
      ...
      <div className="mt-6 grid gap-3 sm:grid-cols-2">
        {["სახელი, აღწერა და ფოტო-ბმულები", "ავტორი და წყაროს ID", "3MF/პროფილის ტექნიკური მონაცემები", "მასალა, წონა და ბეჭდვის დრო", "კატეგორია, ფასი და გამოქვეყნება"].map((item, index) => <div key={item} className="flex items-center gap-3 rounded-2xl bg-hooma-panel/70 p-4 text-sm"><span className="grid h-7 w-7 shrink-0 place-items-center rounded-full bg-white text-xs font-semibold">{index + 1}</span>{item}</div>)}
      </div>
    </form>
  );
}
