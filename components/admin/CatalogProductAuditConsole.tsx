  ...
  return (
    <section className="min-w-0 max-w-full space-y-6 overflow-x-hidden border-t border-hooma-text/10 pt-8">
      ...
      <form action={createJob} className="min-w-0 max-w-full overflow-hidden rounded-[1.75rem] bg-white/75 p-6 shadow-soft">
        <div className="flex items-start gap-3"><span className="rounded-2xl bg-hooma-accent/10 p-3 text-hooma-accent"><Bot size={22} /></span><div><h3 className="text-xl font-semibold">აუდიტ-გაუვლელი პროდუქტების გაშვება</h3><p className="mt-1 text-sm leading-6 text-hooma-muted">AI გამოიყენებს მხოლოდ ერთ მთავარ ფოტოს სახელის, აღწერისა და მიახლოებითი ზომის შესათავაზებლად. მედია, ფერები და AMS არ შეიცვლება; ყველა შედეგი დარჩება მენეჯერის review-სა და დამტკიცებაზე.</p></div></div>
        <div className="mt-5 grid min-w-0 gap-4 xl:grid-cols-[minmax(0,1fr)_minmax(0,1.35fr)_minmax(180px,0.65fr)]"><label className="min-w-0 text-sm font-medium">Catalog Agent<select name="agent_id" required className={inputClass}><option value="">აირჩიე</option>{activeAgents.map((agent) => <option key={agent.id} value={agent.id}>{agent.name}</option>)}</select></label><fieldset className="min-w-0"><legend className="text-sm font-medium">პროდუქტის სტატუსები</legend><div className="mt-2 flex min-w-0 flex-wrap gap-2">{[["active", "გამოქვეყნებული"], ["draft", "Draft"], ["archived", "არქივი"]].map(([value, label]) => <label key={value} className="flex items-center gap-2 rounded-full border border-hooma-text/10 bg-white px-4 py-2.5 text-sm"><input type="checkbox" name="product_statuses" value={value} defaultChecked={value !== "archived"} className="accent-hooma-accent" />{label}</label>)}</div></fieldset><label className="min-w-0 text-sm font-medium">პროდუქტების რაოდენობა<input type="number" name="product_limit" min={1} max={100000} step={1} defaultValue={10} required inputMode="numeric" className={inputClass} /><span className="mt-2 block text-xs font-normal leading-5 text-hooma-muted">აუდიტი ზუსტად ამ რაოდენობამდე გაჩერდება.</span></label></div>
