  ...
  return (
    <section className="space-y-6 border-t border-hooma-text/10 pt-8">
      ...
      <form action={createJob} className="rounded-[1.75rem] bg-white/75 p-6 shadow-soft">
        ...
        <div className="mt-5 grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.35fr)_minmax(180px,0.65fr)]"><label className="text-sm font-medium">Catalog Agent<select name="agent_id" required className={inputClass}><option value="">აირჩიე</option>{activeAgents.map((agent) => <option key={agent.id} value={agent.id}>{agent.name}</option>)}</select></label><fieldset><legend className="text-sm font-medium">პროდუქტის სტატუსები</legend><div className="mt-2 flex flex-wrap gap-2">{[["active", "გამოქვეყნებული"], ["draft", "Draft"], ["archived", "არქივი"]].map(([value, label]) => <label key={value} className="flex items-center gap-2 rounded-full border border-hooma-text/10 bg-white px-4 py-2.5 text-sm"><input type="checkbox" name="product_statuses" value={value} defaultChecked={value !== "archived"} className="accent-hooma-accent" />{label}</label>)}</div></fieldset><label className="text-sm font-medium">პროდუქტების რაოდენობა<input type="number" name="product_limit" min={1} max={100000} step={1} defaultValue={10} required inputMode="numeric" className={inputClass} /><span className="mt-2 block text-xs font-normal leading-5 text-hooma-muted">აუდიტი ზუსტად ამ რაოდენობამდე გაჩერდება.</span></label></div>
