...
export function ProductMediaEditor({
  ...
  return (
    <section className="rounded-[1.75rem] bg-white/75 p-6 shadow-soft">
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-start"><div><p className="text-xs uppercase tracking-[0.2em] text-hooma-muted">Media editor</p><h2 className="mt-2 text-xl font-semibold">ფოტოებისა და ვიდეოს რედაქტირება</h2><p className="mt-2 max-w-3xl text-sm leading-6 text-hooma-muted">ამოიღე არასასურველი მედია, დაამატე ახალი ფაილები და ვარსკვლავით აირჩიე მთავარი ფოტო. პროდუქტის ფასი, სტატუსი და უკვე დამტკიცებული აუდიტი შენარჩუნდება.</p></div><button type="button" disabled={busy} onClick={() => void save()} className="inline-flex shrink-0 items-center justify-center gap-2 rounded-full bg-hooma-text px-5 py-3 text-sm font-semibold text-white disabled:opacity-50">{busy ? <LoaderCircle size={16} className="animate-spin" /> : <Check size={16} />}{busy ? "ინახება..." : "მედიის შენახვა"}</button></div>
