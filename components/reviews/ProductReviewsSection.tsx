...
const dateFormatter = new Intl.DateTimeFormat("ka-GE", { year: "numeric", month: "long", day: "numeric" });
...
export function ProductReviewsSection({
  ...
  const remove = () => {
    if (!window.confirm("ნამდვილად წავშალოთ შენი შეფასება?")) return;
  ...
  return (
    <section id="reviews" className="mt-14 scroll-mt-32 rounded-[2rem] border border-hooma-text/10 bg-white/75 p-5 shadow-sm sm:p-7 lg:p-9">
      <div className="flex flex-col justify-between gap-5 border-b border-hooma-text/10 pb-6 md:flex-row md:items-end">
        <div><p className="text-xs font-semibold uppercase tracking-[0.18em] text-hooma-accent">მომხმარებლების გამოცდილება</p><h2 className="mt-2 text-3xl font-semibold tracking-tight">შეფასებები და კომენტარები</h2><p className="mt-2 text-sm leading-6 text-hooma-muted">რეალური მყიდველების შეფასებები · {productName}</p></div>
      ...
      <div className="mt-7 grid items-start gap-8 lg:grid-cols-[360px_minmax(0,1fr)]">
        <aside className="rounded-2xl bg-hooma-panel p-5 lg:sticky lg:top-32">
          <div className="flex items-center gap-2"><MessageSquareText size={18} className="text-hooma-accent" /><h3 className="font-semibold">შენი გამოცდილება</h3></div>
          {!allowReview ? <p className="mt-4 text-sm leading-6 text-hooma-muted">სატესტო Preview-ზე შეფასების დამატება გამორთულია.</p> : !context.authenticated ? <div className="mt-4"><p className="text-sm leading-6 text-hooma-muted">შეფასების დასაწერად ანგარიშში შესვლა საჭიროა.</p><Link href={`/login?next=/product/${slug}%23reviews`} className="mt-4 inline-flex rounded-xl bg-hooma-text px-4 py-2.5 text-sm font-semibold text-white">ანგარიშში შესვლა</Link></div> : !context.eligible ? <div className="mt-4 rounded-xl border border-hooma-text/10 bg-white/70 p-4"><p className="text-sm font-semibold">შეფასება ხელმისაწვდომი გახდება მიწოდების შემდეგ</p><p className="mt-2 text-xs leading-5 text-hooma-muted">ასე თითოეულ კომენტარს რეალურ შეკვეთასთან ვაკავშირებთ და შეფასებები სანდო რჩება.</p></div> : <div className="mt-5">
            {context.review ? <p className="mb-3 text-xs font-semibold text-hooma-accent">შენი არსებული შეფასების რედაქტირება</p> : null}
