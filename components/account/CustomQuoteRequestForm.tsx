...
export function CustomQuoteRequestForm() {
  ...
  const chooseFiles = (selected: FileList | null) => {
    ...
    if (invalid) {
      setMessage(`ფაილი “${invalid.name}” არასწორი ფორმატისაა ან 100MB-ს აღემატება.`);
      return;
    }
    if (next.reduce((sum, file) => sum + file.size, 0) > maxTotalSize) {
      setMessage("ფაილების ჯამური ზომა 250MB-ს არ უნდა აღემატებოდეს.");
      return;
    }
    setMessage("");
    setFiles(next);
  ...
  const submit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!files.length) { setMessage("ატვირთე მინიმუმ ერთი ფაილი."); return; }
    ...
    if (!user) { setBusy(false); setMessage("ფაილის გასაგზავნად ჯერ ანგარიშში შედი."); return; }

    setProgress("დაცული ატვირთვა მზადდება...");
    ...
    for (let index = 0; index < files.length; index += 1) {
      ...
      if (error) {
        ...
        setBusy(false);
        setProgress("");
        setMessage(`ატვირთვა ვერ დასრულდა: ${error.message}`);
        return;
      }
      uploaded.push({ path, originalName: file.name, mimeType: file.type, size: file.size });
    }

    setProgress("მოთხოვნა იგზავნება შეფასებისთვის...");
  ...
  return (
    <form onSubmit={submit} className="space-y-5 rounded-[1.5rem] border border-hooma-text/10 bg-white/80 p-5 shadow-sm sm:p-6">
      <div><p className="text-xs font-semibold uppercase tracking-[0.2em] text-hooma-accent">New request</p><h2 className="mt-2 text-2xl font-semibold">მიიღე ინდივიდუალური ფასი</h2><p className="mt-2 text-sm leading-6 text-hooma-muted">ატვირთე მოდელი, ნახაზი ან ფოტო. ოპერატორი შეამოწმებს დამზადების შესაძლებლობას, მასალას, დროსა და ფასს.</p></div>

      <div className="grid gap-4 sm:grid-cols-2">
        <label className="text-sm font-medium sm:col-span-2">რას ამზადებ?<input name="title" required minLength={3} maxLength={120} placeholder="მაგ. კედლის სამაგრი კონკრეტული ზომით" className="mt-2 w-full rounded-xl border border-hooma-text/10 bg-white px-4 py-3 outline-none focus:border-hooma-accent" /></label>
        <label className="text-sm font-medium">რაოდენობა<input name="quantity" type="number" min="1" max="100" defaultValue="1" required className="mt-2 w-full rounded-xl border border-hooma-text/10 bg-white px-4 py-3 outline-none focus:border-hooma-accent" /></label>
        <label className="text-sm font-medium">ზომები<input name="dimensions" maxLength={500} placeholder="მაგ. 120 × 60 × 25 მმ" className="mt-2 w-full rounded-xl border border-hooma-text/10 bg-white px-4 py-3 outline-none focus:border-hooma-accent" /></label>
        <label className="text-sm font-medium">სასურველი მასალა<input name="material_preference" maxLength={120} placeholder="თუ არ იცი, ცარიელი დატოვე" className="mt-2 w-full rounded-xl border border-hooma-text/10 bg-white px-4 py-3 outline-none focus:border-hooma-accent" /></label>
        <label className="text-sm font-medium">სასურველი ფერი<input name="color_preference" maxLength={120} placeholder="მაგ. შავი" className="mt-2 w-full rounded-xl border border-hooma-text/10 bg-white px-4 py-3 outline-none focus:border-hooma-accent" /></label>
        <label className="text-sm font-medium sm:col-span-2">დეტალური აღწერა<textarea name="description" required minLength={10} maxLength={3000} rows={5} placeholder="აღწერე გამოყენება, დატვირთვა, თავსებადობა და სხვა მნიშვნელოვანი მოთხოვნები" className="mt-2 w-full rounded-[1.25rem] border border-hooma-text/10 bg-white px-4 py-3 outline-none focus:border-hooma-accent" /></label>
      </div>

      <div>
        <p className="text-sm font-medium">ფაილები</p>
        <label className="mt-2 flex cursor-pointer flex-col items-center rounded-[1.25rem] border border-dashed border-hooma-text/20 bg-hooma-background px-5 py-8 text-center transition hover:border-hooma-accent/60">
          <Upload size={24} className="text-hooma-accent" /><span className="mt-3 text-sm font-semibold">აირჩიე მაქსიმუმ 5 ფაილი</span><span className="mt-1 text-xs leading-5 text-hooma-muted">3MF, STL, STEP, STP, OBJ, ZIP, PDF ან ფოტო · 100MB თითო ფაილი</span>
      ...
      <div className="rounded-xl border border-amber-200 bg-amber-50 p-4 text-xs leading-5 text-amber-900">ფაილები ინახება private storage-ში და ავტომატურად არ გაეშვება ბეჭდვაზე. ოპერატორი ჯერ ამოწმებს ფაილს, უსაფრთხოებასა და წარმოების პარამეტრებს.</div>
      {message ? <p className="rounded-xl bg-hooma-panel p-4 text-sm leading-6">{message}</p> : null}
      <button type="submit" disabled={busy} className="flex min-h-12 w-full items-center justify-center gap-2 rounded-full bg-hooma-text px-5 text-sm font-semibold text-white transition hover:bg-hooma-accent disabled:cursor-not-allowed disabled:opacity-50">{busy ? <><LoaderCircle size={17} className="animate-spin" />{progress || "იგზავნება..."}</> : "ფასის მოთხოვნის გაგზავნა"}</button>
    </form>
  );
}
