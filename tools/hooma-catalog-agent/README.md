...
Provider HTTP failures stop the current job after recording the affected product, except product-specific image/file download failures, which fail only that product and continue without retrying the paid request. Three consecutive invalid/refused model outputs trip a circuit breaker, and a completed analysis that Hooma permanently rejects is quarantined and stops the job immediately. This prevents a broken model name, schema, credential, or worker/API contract from consuming the entire catalog.
