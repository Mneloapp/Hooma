...
7. Choose the downloaded product package folder once. Hooma matches the extracted category/subcategory path against its active category tree, fills the technical and color fields and attaches its image/video files automatically. Copy already captured in Georgian skips the Google Cloud translation request; non-Georgian copy can still use the configured server translation fallback.
