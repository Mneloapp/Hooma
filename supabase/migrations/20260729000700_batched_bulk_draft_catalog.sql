-- Process a very large catalog in short transactions so PostgREST and database
-- statement timeouts cannot roll back the entire Draft reset.

create or replace function public.move_catalog_products_to_draft_batch_v1(
  actor_profile_id uuid,
  confirmation_token text,
  requested_batch_size integer default 2000
)
returns jsonb
language plpgsql
security definer
...
as $$
declare
  resolved_batch_size integer := least(greatest(coalesce(requested_batch_size, 2000), 1), 5000);
...
revoke all on function public.move_catalog_products_to_draft_batch_v1(uuid, text, integer)
  from public, anon, authenticated;
grant execute on function public.move_catalog_products_to_draft_batch_v1(uuid, text, integer)
  to service_role;
