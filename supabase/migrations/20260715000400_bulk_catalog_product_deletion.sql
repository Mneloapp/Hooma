-- Atomic bulk product deletion for catalog staff. Historical order and deal records stay protected.
...
begin
  ...
  where product.id = any(normalized_ids)
    and exists (
      ...
      where item.product_id = product.id or variant.product_id = product.id
    )
  limit 1;

  if blocked_product_name is not null then
    raise exception 'Product is referenced by an order and cannot be deleted: %', blocked_product_name;
  end if;
  ...
  select
    ...
    'product',
    deleted_item->>'id',
    deleted_item || jsonb_build_object('deletion_mode', 'bulk', 'batch_size', array_length(normalized_ids, 1))
