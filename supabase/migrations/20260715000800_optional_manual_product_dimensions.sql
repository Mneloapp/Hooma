-- Manual product dimensions are optional; when provided they remain strictly validated.
...
begin
  ...
  if selected_dimensions is not null and (
    jsonb_typeof(selected_dimensions) <> 'object'
    or coalesce((selected_dimensions->>'x')::numeric, 0) <= 0
    or coalesce((selected_dimensions->>'y')::numeric, 0) <= 0
    or coalesce((selected_dimensions->>'z')::numeric, 0) <= 0
  ) then
